...
export default function CheckoutButton({ onCheckout, disabled, isLoading } : Props) {
    ...
    if (!isAuthenticated) return <Button onClick={onLogin} type="button" className="bg-orange-500 flex-1">Log In to
        check out</Button>
        ...
        <Dialog>
            ...
            <DialogContent className="max-w-[425px] md:min-w-[700px] bg-gray-50">
                <UserProfileForm currentUser={currentUser} onSave={onCheckout} isLoading={isGetUserLoading}
                                 title="Confirm Delivery Details" buttonText="Continue to Payment"/>
            </DialogContent>
        </Dialog>
    )
}